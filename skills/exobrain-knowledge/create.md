...
Arguments: `<name>` (folder name, lowercase-hyphenated, under `knowledge/`) and `<type>` (the domain's kind — `system`, `process`, `expertise`/`reference`, `product`/`project`, or another type that fits; see existing `knowledge/*/README.md` for the vocabulary in use). For a parallel-build WIP domain, use [`distill --wip`](distill.md) instead — those are grilled into existence at a definition altitude, not built from source search.
...
2. Read [`knowledge/exobrain/domains.md`](../../knowledge/exobrain/domains.md) (layout, section design), [`knowledge/exobrain/authoring.md`](../../knowledge/exobrain/authoring.md) (editorial rules), and existing `knowledge/*/README.md` (overlap, cross-references).
