...
2. **Set and keep the README `summary:` current** — the one-line frontmatter field stating the domain's scope. It's pulled verbatim into the auto-loaded **domains index** ([`knowledge/exobrain/domains.md`](../../knowledge/exobrain/domains.md) → "The README `summary:` and the domains index"), so every mode owns it: `create`/`distill` write it when scaffolding; `curate`/`update` refresh it whenever a pass changes what the domain covers. One line — let the body's TL;DR elaborate.
