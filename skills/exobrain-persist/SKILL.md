...
1. **Get onto a branch.** If you're not already in a worktree/branch, create one off the default branch with `scripts/create-worktree.sh <branch>` and `cd` into the path it prints. If work began in-place on `main`, move it to the branch first.
