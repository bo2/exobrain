...
description: "Clear the authoring-review findings that unattended lands record in merged PR bodies instead of blocking on. Repairs one PR per session, applying only the fix each finding prescribes, then labels the PR repaired. Use when asked to repair or clear recorded authoring findings, or as the payload of a scheduled repair job."
---

# Repair authoring findings

An unattended land — a chat turn's detached land, or one the sweep finishes — writes the authoring review's findings into the PR body rather than blocking on them (`exobrain-persist` § Land). This skill is what later acts on those findings.
...
7. **Label the source PR** `findings-repaired` (`gh pr edit <number> --add-label findings-repaired`, creating the label once with `gh label create`), so the next run doesn't pick it up again. Add a comment naming the repair PR by full URL, prefixed 🤖 (`AGENTS.md` § Git workflow).
