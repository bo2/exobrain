...
- **Start new work from current trunk** — before creating a worktree, fast-forward the default branch in the main checkout (`git pull --ff-only`) so you branch off current state, not a stale base. If it can't fast-forward (dirty or diverged), work off the current state and say so — never force. Skip when resuming work already in progress.
- **Worktree-first — create a worktree before touching files.** Every logical change gets its own branch and worktree; never commit on the default branch directly, even for a quick fix. `scripts/create-worktree.sh <branch>` names it, branches off the default branch, and symlinks `.env*`/`.exobrain.json`; `cd` into the path it prints.
