...
- **Are independent.** Each owns its own tree. No upstream remote, no merge, no conflicts — `exobrain-update` caches the seed in a gitignored `src/exobrain-seed/` (pull if present, else clone) and reads it from there. A local cache, not a tracked remote: nothing to merge, nothing that pulls framework code in automatically.
...
Every change worth propagating is published as a dated **pattern-card** under `domains/exobrain/feed/` — a problem, a pattern, optionally the files it touches, and adapt notes. The feed is the changelog an instance reads to update. Format + ledger: [`feed/README.md`](feed/README.md). Each card carries a stable ID so provenance survives even across divergence.

## The update workflow

The `exobrain-update` skill is the single way an instance moves forward — there is no `git pull` of framework code:
...
2. **Diff** the feed's card IDs against this instance's adoption ledger; the unadopted cards are the changes since last update.
...
4. Seed the minimum: `AGENTS.md`, a `domains/exobrain/` meta-domain, one person scope, an empty `workspaces/`, and the `exobrain-update` skill.
5. Seed the adoption ledger with the cards the seed currently publishes — you built from them — so `exobrain-update` carries you forward from there.
