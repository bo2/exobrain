...
The **self-test suites** are *not* seed-local — they're the global
...
built instance. That includes the deterministic `unit/` harnesses for
`connect-agent.sh`, `skills-registry.sh`, and `authoring-review.sh`: those scripts
are framework body, inherited by every instance, so their tests travel with them.
Run them against this checkout directly — `skills/exobrain-tests/unit/run.sh`.
