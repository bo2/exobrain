...
| `scripts/persist.sh` | The mechanical land as one idempotent command, run from inside the worktree: timeline rows → commit → validator → authoring review → push → PR → squash-merge → fast-forward main → cleanup. On a machinery diff it runs the unit suite itself and lands only with `--machinery-verified`; `--detach` returns after the commit and lands in the background; `--context` carries a session handover into the PR body; `--sweep` lands claimed and quiet worktrees and fails on one left stale; `--dry-run` prints the plan. An unattended land (detached or swept) posts authoring findings as a comment review on the PR instead of blocking. Semantics in the `exobrain-persist` skill. |
