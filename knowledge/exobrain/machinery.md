...
| `skills/exobrain-tests/unit/test-validator-checks.sh` | Harness for `validate-exobrain.sh`'s whole-tree checks — UPPERCASE names, bash-4 constructs, unguarded empty-array expansions, the COMPAT marker scan — each asserting the exact set of violations its fixture raises. |
| `skills/exobrain-tests/unit/test-skills-validate.sh` | Harness for `skills-validate.sh` — every registry error it raises, undeclared skill dirs as info, its exit codes, and the linker, clone, runtime and cache dirs it never walks. |
| `skills/exobrain-tests/unit/test-skills-status.sh` | Harness for `skills-status.sh --all` — every catalog column lands under its own heading whatever fields a declaration omits, and an external declaration reads as external. |
