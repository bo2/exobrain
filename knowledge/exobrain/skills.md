...
**A shared skill must earn its reach.** Declaring one at a shared scope loads it for everyone whose chain includes that scope, so a new one carries committed proof it pays for that: a `schedule.json` registration for a skill a periodic job drives, or a test, eval, or `exobrain-ab` result committed **inside the skill's own directory** — for a behavior-shaping skill, an A/B result showing it actually moves behavior. Proof lives with the skill rather than as a link to the workspace that produced it, so it can't go stale out from under it. Without proof the skill belongs under a person scope's `skills/`, where it imposes on no one and an interested colleague can still opt in. `scripts/authoring-review.sh` enforces this at the land. What counts as new is a widening of reach, not a new string: a skill already declared in a registry keeps its standing there when renamed, while one moved from a person scope into a shared one faces the gate.
