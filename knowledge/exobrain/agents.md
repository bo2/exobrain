...
- The Codex connector writes its context and skills inside the checkout and leaves personal Codex home files and symlinks untouched.
...
3. **Link always-tier skills** — symlink each into the agent's skills dir as `<name>.<scope-owner>/`. Most agents read it from their context surface (`.claude/skills`, `~/.openclaw/workspace/skills`); Codex scans a repo-local `.agents/skills`, so its skills link there — out of the global `~/.codex`, scoped to this repo. OpenClaw honors a workspace skill symlink only when its target root is trusted in its config — step 8 keeps that list current.
4. **Fetch external skills** — route to `skills/` (always) or `skills-optional/` (optional).
5. **Generate the indexes** — `optional-skills.md` (optional-tier skills), `tools-index.md` (visible tool docs), `knowledge-index.md` (domains + their summaries, then a section per mount — [`mounts.md`](mounts.md)).
6. **Compose + inject** — deliver each connected scope's `AGENTS.md` (+ agent sidecar, shallow→deep) and the generated indexes. Claude writes `.claude/connected-scopes.md` — a manifest of `@-import`s to the live source specs — and a `.claude/CLAUDE.md` that `@-import`s that manifest plus each index; Codex writes the self-contained in-repo `AGENTS.override.md` (root scope and root sidecar inlined, since it replaces `AGENTS.md`); OpenClaw inlines the same specs and indexes into a `USER.md` marker block, with its root sidecar prepended (the root `AGENTS.md` itself loads natively), and gets a second marker block in its workspace `.gitignore` — the runtime writes raw capture into `memory/` (a near-verbatim transcript per session reset, plus the dreaming sweep's corpus and candidate logs), and a workspace with a remote would otherwise publish all of it permanently. Files already tracked need untracking by hand; the run says so.
...
8. **Reconcile the OpenClaw runtime config** (openclaw only) — through `openclaw config set`: the real parent dir of every linked skill joins `skills.load.allowSymlinkTargets`, `skill_workshop` joins `tools.deny`, `skills.workshop.autonomous.mode` becomes `off` (root `OPENCLAW.md` § Skills are exobrain files), the bootstrap budgets (`agents.defaults.bootstrapMaxChars` / `bootstrapTotalMaxChars`) are raised to floors that hold the injected `USER.md` — past the per-file budget its tail, the knowledge index, is silently dropped — and `memory.search.extraPaths` gains this checkout's `knowledge/` and each listed mounted domain, so the runtime's semantic recall reaches the domains and not only its own memory files. The paths are positive per-depth globs, since the indexer cannot exclude a path segment and a domain's `_raw/`/`_meta/` must stay out; indexing sends the matched files to the runtime's configured embedding provider. Lists are unioned, so entries the human added stay, and a larger budget stays; nothing is written when the config already matches; a missing CLI is reported and the connect carries on. `OPENCLAW_BIN` overrides the binary.
9. **Run scope hooks** — a connected scope extends the connect with setup of its own. Each scope in the chain carrying an executable `scripts/connect-agent.sh` (every agent) or `scripts/connect-agent.<agent>.sh` (that agent alone) gets it run, shallow→deep, as `<hook> <agent> <target-dir> <scope-dir>`; both run when both exist, universal first. The global scope is skipped — its `scripts/connect-agent.sh` is the connector. A hook that fails is reported and the connect carries on.

`--relink` repeats steps 2–9 without prompting and writes neither marker nor config (a `--relink` for an unmarked agent exits before any of this; `--relink` with no agent named runs it once per marked agent, and takes no other flag); `--configure` re-resolves identity (the wizard, or the identity flags); `--wire-sandbox` runs steps 2–6 and stops before hooks, the OpenClaw config, and scope hooks — real wiring for a throwaway copy (a test sandbox, a CI checkout), so a wire-sandboxed checkout counts as connected for claude. For openclaw the mode refuses to run unless `OPENCLAW_WORKSPACE` is set, so the real home workspace is never the implicit target — the guard checks that the variable is set, not where it points, so point it at a throwaway dir.
...
`create-worktree.sh` calls `link-worktree-context.sh <main> <worktree>` to inherit the generated markdown and Claude's per-machine `.claude/settings.local.json` (permission allowlists, gitignored), and to create real Codex skill directories with individual skill links. In-tree skill links prefer the worktree's own source, so a branch's skill edits are what the agent loads there; fetched skills use the main checkout's cache when the worktree has no copy. Existing destination files are preserved, and re-running the helper repairs missing links in an existing worktree. Mounts resolve through the main checkout ([`mounts.md`](mounts.md)).

Wiring Codex inside a worktree replaces the inherited `AGENTS.override.md` symlink with a local composition and leaves the main checkout's file untouched. The healthcheck checks Codex's context and resolved always-tier skills in the active checkout; connection markers and trunk freshness come from the main checkout.
