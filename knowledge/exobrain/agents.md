...
8. **Reconcile the OpenClaw runtime config** (openclaw only) — through `openclaw config set`: the real parent dir of every linked skill joins `skills.load.allowSymlinkTargets`, `skill_workshop` joins `tools.deny`, and `skills.workshop.autonomous.mode` becomes `off` (root `OPENCLAW.md` § Skills are exobrain files). Lists are unioned, so entries the human added stay; nothing is written when the config already matches; a missing CLI is reported and the connect carries on. `OPENCLAW_BIN` overrides the binary.
