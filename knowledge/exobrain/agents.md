...
8. **Reconcile the OpenClaw runtime config** (openclaw only) — through `openclaw config set`: the real parent dir of every linked skill joins `skills.load.allowSymlinkTargets`, `skill_workshop` joins `tools.deny`, `skills.workshop.autonomous.mode` becomes `off` (root `OPENCLAW.md` § Skills are exobrain files), the bootstrap budgets (`agents.defaults.bootstrapMaxChars` / `bootstrapTotalMaxChars`) are raised to floors that hold the injected `USER.md` — past the per-file budget its tail, the knowledge index, is silently dropped — and `memory.search.extraPaths` gains this checkout's `knowledge/` and each listed mounted domain, so the runtime's semantic recall reaches the domains and not only its own memory files. The paths are four positive depth globs, since the indexer cannot exclude a path segment and a domain's `_raw/`/`_meta/` must stay out; indexing sends the matched files to the runtime's configured embedding provider. Lists are unioned, so entries the human added stay, and a larger budget stays; nothing is written when the config already matches; a missing CLI is reported and the connect carries on. `OPENCLAW_BIN` overrides the binary.
